<!-- <template>
  <div class="btn-group btn-with-tooltip-group" ref="filterDropdownRef" data-toggle="tooltip"
    :data-title="$t('filter_by')">
    <button type="button" :class="activeFilterId || builderRules.length ? 'active' : ''"
      class="btn btn-default dropdown-toggle sm:tw-max-w-xs tw-truncate" data-toggle="dropdown" aria-haspopup="true"
      aria-expanded="false">
      <i class="fa fa-filter" aria-hidden="true"></i>
      {{ activeFilterId ? activeFilter.name : "" }}
    </button>

    <ul class="dropdown-menu dropdown-menu-right width300 tw-max-h-[500px] tw-overflow-y-auto" @click.stop="">
      <div class="tw-py-2 tw-px-4 tw-space-x-2 tw-divide-x tw-divide-solid tw-divide-neutral-300">
        <a href="#" @click="newFilter">{{ $t("filter_new") }}</a>
        <a href="#" @click="clearActiveFilter" v-show="activeFilterId" class="tw-pl-2">
          {{ $t("filter_clear_active") }}
        </a>
        <a href="#" @click="initiateEditFilter" class="tw-pl-2" v-show="activeFilterId || builderRules.length > 0">
          {{ $t("filter_edit") }}
        </a>
      </div>
      <li class="divider"></li>
      <div v-if="localSavedFilters.length === 0"
        class="tw-px-4 tw-pt-4 tw-pb-3 tw-text-balance tw-text-center tw-text-neutral-500">
        {{ $t("no_filters_found") }}
      </div>
      <li v-for="filter in localSavedFilters" :key="filter.id" :class="filter.id == activeFilterId ? 'active' : ''">
        <a href="#" @click.prevent="setFilterActive(filter), hideFiltersDropdown()">
          <i class="fa-regular fa-star tw-text-primary-600" v-if="filter.is_default" />
          {{ filter.name }}
        </a>
      </li>
    </ul>
      ...
      <div class="modal-dialog modal-lg tw-max-w-[48rem]" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <div class="tw-px-3">
              <select class="selectpicker tw-mb-3" v-model="selectedGroup" @change="onGroupChange" data-width="100%"
                :data-none-selected-text="$t('select_group')">
                <option v-for="group in groups" :key="group.value" :value="group.value">
                  {{ group.label }}
                </option>
              </select>

              <select class="selectpicker" :id="rulesSelectId" data-width="100%" data-live-search="true" value=""
                name="tax" :data-none-selected-text="$t('filter_add_rule')">
                <option v-for="(rule, index) in visibleAvailableRules" :key="rule.id + index" :value="rule.id">
                  {{ rule.label }}
                </option>
              </select>
              ...
              <div>
                <div class="tw-mt-8">
                  <span v-show="builderRules.length > 1"
                    ...
                    <div class="radio radio-primary radio-inline">
                      <input type="radio" id="match_type_and" :name="`match_${id}`" value="and"
                    ...
                    <div class="radio radio-primary radio-inline">
                      <input type="radio" id="match_type_or" :name="`match_${id}`" value="or"
                        v-model="localMatchType" />
                ...
                <div
                  ...
                  <div v-for="(rule, index) in builderRules" :key="rule.id" :class="[
                    'tw-block',
                    builderRules.length - 1 !== index ? 'tw-mb-6' : '',
                  ]">
                    <AppFiltersRule :rule="rule" :match-type="localMatchType" :index="index"
                      @remove-requested="removeRule(index)" @operator-selected="builderRules[index].operator = $event"
                      @update-rule-value="builderRules[index].value = $event" @has-dynamic-value="
                        builderRules[index].has_dynamic_value = $event
                        " @has-errors="rulesErrors[rule.id + index] = $event" />
                  </div>
                </div>
              </div>
              <div v-if="filterBeingSaved || (activeFilter && canUpdate)" class="tw-mt-4">
                <div class="form-group">
                  <label for="filter_name">
                    <span class="tw-text-danger-500">*</span>
                    {{ $t("filter_name") }}
                  </label>
                  <input type="email" class="form-control" id="filter_name" v-model="filterName" />
                </div>
                <div class="checkbox checkbox-primary">
                  <input type="checkbox" :id="id + 'ShareFilter'" :disabled="hasRulesAppliedThatAreNotVisibleToAllUsers"
                    v-model.boolean="filterIsShared" />
                  <label :for="id + 'ShareFilter'">
                    {{ $t("filter_share") }}
                  </label>
                </div>
                <span v-if="hasRulesAppliedThatAreNotVisibleToAllUsers" class="tw-text-sm -tw-mt-2.5 tw-block tw-ml-6">
                  {{ $t("filter_cannot_be_shared") }}
                </span>
                <div class="checkbox checkbox-primary">
                  <input type="checkbox" :id="id + 'DefaultFilter'" @change="handleDefaultInputChange"
          ...
          <div class="modal-footer">
            <div class="tw-flex tw-items-center tw-space-x-3">
              <button v-if="activeFilterId && canDelete" type="button" class="btn btn-danger btn-sm"
                data-toggle="tooltip" :data-title="$t('filter_delete')" @click="deleteFilter(activeFilterId)">
                <i class="fa-regular fa-trash-can" v-show="!deleteBeingConfirmed" />
                {{ deleteBeingConfirmed ? $t("confirm") : "" }}
              </button>
              <div class="tw-flex-1 tw-flex tw-items-center tw-space-x-2 tw-self-end tw-justify-end">
                <span v-if="!activeFilter" class="relative tw-mt-1">
                  <span class="tw-font-medium tw-mr-3 tw-absolute -tw-left-20">
                    {{ $t("filter_save") }}
                  </span>
                  <div class="onoffswitch">
                    <input type="checkbox" :id="id + 'SaveFilter'" class="onoffswitch-checkbox"
                      v-model.boolean="filterBeingSaved" />
                    <label class="onoffswitch-label" :for="id + 'SaveFilter'" />
                  </div>
                </span>

                <button v-if="activeFilterId && !canUpdate" type="button" class="btn btn-sm btn-secondary" @click="
                ...
                ]" :disabled="isApplyDisabled" @click="applyHandler">
                  {{
...
const groups = computed(() => {
  ...
  props.availableRules.forEach(rule => {
    if (!rule.group) return;
    if (!unique.has(rule.group)) {
      unique.set(rule.group, {
        value: rule.group,
        label: rule.group.charAt(0).toUpperCase() + rule.group.slice(1)
      });
    }
  });
...
const visibleAvailableRules = computed(() =>
  ...
  props.availableRules.filter(
    (r) =>
      r.visible_to_all === true &&
      (!selectedGroup.value || r.group === selectedGroup.value)
  )
);
...
function setFilterActive(filter, reloadTable = true) {
  ...
  // Set selected group based on first rule
  if (builderRules.value.length > 0) {
    selectedGroup.value = builderRules.value[0].group;
  } else {
    selectedGroup.value = '';
  }
  ...
  nextTick(() => {
    $(rulesSelecSelector).selectpicker('refresh');
  });
}
...
function handleRulesSelectChange(e) {
  ...
  if (!ruleId || !selectedGroup.value) return;

  // Find rule in props.availableRules that matches id AND selected group
  const rule = props.availableRules.find(
    (r) => r.id === ruleId && r.group === selectedGroup.value
  );
  ...
  if (rule) {
    addRule({
      ...
      operator: rule.operator ?? (rule.operators ? rule.operators[0] : ''),
      has_dynamic_value: rule.has_dynamic_value ?? false,
    });
  }
...
function onGroupChange() {
  // clear selected rules when group changes
  builderRules.value = [];
  $(rulesSelecSelector).selectpicker("val", "");
}

watch([selectedGroup, visibleAvailableRules], () => {
  nextTick(() => {
    $(rulesSelecSelector).selectpicker('refresh');
    $(rulesSelecSelector).selectpicker('val', ''); // reset dropdown
  });
});
</script> -->
