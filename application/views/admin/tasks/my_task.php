    ...
    <div class="modal-dialog" role="document">
      ...
      echo 'selected';
                    ...
                    <?php $rel_id_custom_field = (isset($task) ? $task->id : false); ?>
                        ...
                        <div class="form-group col-md-12" id="follow_sop_wrapper" style="display:none; padding-left: 1px;">
                            <label class="control-label">
                                Did you add the details to the Attribution Issue Sheet?
                                 <a target="_blank" style="color: black;" href="https://docs.google.com/spreadsheets/d/10IPrTV0sL9jOiXnzBgbqlpnBCoBeoJN6jLAGBCUfce8/edit?pli=1&gid=725702296#gid=725702296">
                                    <i class="fa-solid fa-circle-question" style="color:black;"></i>
                                 </a>
                                </label>
                            <div>
                                <label class="radio-inline">
                                    <?php $follow_sop = isset($task) ? $task->follow_sop : ''; ?>
                        ...
                        <div class="form-group col-md-12" id="follow_sop_no_question_wrapper" style="display: none; padding-left: 1px;">
                            <?php 
                                echo render_textarea('why_not' , 'Why not?', isset($task) ? $task->why_not : '');
                            ?>
                        </div>


                        <div class="form-group col-md-12" id="did_add_automation_issue_wrapper" style="display:none; padding-left: 1px;">
                            <label class="control-label">
                                Did you add the details to the Automation Issue Sheet?
                                 <a target="_blank" style="color: black;" href="https://docs.google.com/spreadsheets/d/10IPrTV0sL9jOiXnzBgbqlpnBCoBeoJN6jLAGBCUfce8/edit?pli=1&gid=1170126395#gid=1170126395">
                                    <i class="fa-solid fa-circle-question" style="color:black;"></i>
                                 </a>
                                </label>
                            <div>
                                <label class="radio-inline">
                                    <?php $did_add_automation_issue = isset($task) ? $task->did_add_automation_issue : ''; ?>
                                    <input type="radio" name="did_add_automation_issue" value="1" <?php echo $did_add_automation_issue == 1 ? 'checked' : ''; ?>> Yes
                                </label>
                                <label class="radio-inline">
                                    <input type="radio" name="did_add_automation_issue" value="0" <?php echo $did_add_automation_issue == 0 ? 'checked' : ''; ?>> No
                                </label>
                            </div>
                        </div>
                        ...
                        <div class="form-group col-md-12" id="automation_issue_yes_question_wrapper" style="display:none; padding-left: 1px;">
                            <?php 
                                echo render_input('auto_issue_link_to_contact_opp_in_ghl' , 'Link To Contact/Opp in GHL', isset($task) ? $task->auto_issue_link_to_contact_opp_in_ghl : '');

                                echo render_date_input('auto_issue_date_contact_entered_in_ghl' , 'Date Contact Entered in GHL', isset($task) ? _d($task->auto_issue_date_contact_entered_in_ghl) : '');

                                echo render_input('auto_issue_screenshot_or_loom_url' , 'Screenshot and/or Loom URL', isset($task) ? $task->auto_issue_screenshot_or_loom_url : '');
                            ?>
                        </div>

                        <div class="form-group col-md-12" id="automation_issue_no_question_wrapper" style="display: none; padding-left: 1px;">
                            <?php 
                                echo render_textarea('auto_issue_why_not' , 'Why not?', isset($task) ? $task->auto_issue_why_not : '');
                            ?>
                        </div>
    ...
    <script>
        ...
        $(function() {
            ...
            function shouldValidateSOP(){
                ...
                return (val10 == 'Issue' && val1 == 'Attribution');
            }

            function shouldValidateSOPYesFields() {
                let val10  = $('[name="custom_fields[tasks][10]"]').val();
                let val1   = $('[name="custom_fields[tasks][1]"]').val();
                let sopVal = $('input[name="follow_sop"]:checked').val();

                return (val10 == 'Issue' && val1 == 'Attribution' && sopVal == '1');
            }

            function shouldValidateSOPNoField() {
                let val10  = $('[name="custom_fields[tasks][10]"]').val();
                let val1   = $('[name="custom_fields[tasks][1]"]').val();
                let sopVal = $('input[name="follow_sop"]:checked').val();

                return (val10 == 'Issue' && val1 == 'Attribution' && sopVal == '0');
            }

            function shouldValidate_did_add_automation_issue(){
                let val10  = $('[name="custom_fields[tasks][10]"]').val();
                let val1   = $('[name="custom_fields[tasks][1]"]').val();
                return (val10 == 'Issue' && val1 == 'Automation');
            }

            function shouldValidate_automation_issue_YesFields() {
                let val10  = $('[name="custom_fields[tasks][10]"]').val();
                let val1   = $('[name="custom_fields[tasks][1]"]').val();
                let did_add_automation_issue = $('input[name="did_add_automation_issue"]:checked').val();

                return (val10 == 'Issue' && val1 == 'Automation' && did_add_automation_issue == '1');
            }

            function shouldValidate_automation_issue_NoField() {
                let val10  = $('[name="custom_fields[tasks][10]"]').val();
                let val1   = $('[name="custom_fields[tasks][1]"]').val();
                let did_add_automation_issue = $('input[name="did_add_automation_issue"]:checked').val();

                return (val10 == 'Issue' && val1 == 'Automation' && did_add_automation_issue == '0');
            }
            ...
            appValidateForm($('#task-form'), {
                ...
                follow_sop: {
                    required: function () {
                        return shouldValidateSOP();
                    }
                },
                link_to_contact_opp_in_ghl: {
                    required: function () {
                        return shouldValidateSOPYesFields();
                    }
                },
                ...
                why_not: {
                    required: function () {
                        return shouldValidateSOPNoField();
                    }
                },
                did_add_automation_issue: {
                    required: function () {
                        return shouldValidate_did_add_automation_issue();
                    }
                },
                auto_issue_link_to_contact_opp_in_ghl: {
                    required: function () {
                        return shouldValidate_automation_issue_YesFields();
                    }
                },

                auto_issue_date_contact_entered_in_ghl: {
                    required: function () {
                        return shouldValidate_automation_issue_YesFields();
                    }
                },

                auto_issue_screenshot_or_loom_url: {
                    required: function () {
                        return shouldValidate_automation_issue_YesFields();
                    }
                },
                ...
                auto_issue_why_not: {
                    required: function () {
                        return shouldValidate_automation_issue_NoField();
                    }
                },
            }, task_form_handler);
            ...
            function toggleFollowSOP(){
                ...
                let val10 = $('[name="custom_fields[tasks][10]"]').val();
                let val1 = $('[name="custom_fields[tasks][1]"]').val();
                $("#follow_sop_no_question_wrapper").hide();
             ...
             // fucntion to show task type and department related field as a conditinal field
            function toggle_automation_issue(){
                
                let val10 = $('[name="custom_fields[tasks][10]"]').val();
                let val1 = $('[name="custom_fields[tasks][1]"]').val();
                $("#follow_sop_no_question_wrapper").hide();
                if (val10 == 'Issue' && val1 == 'Automation') {
                    $('#did_add_automation_issue_wrapper').show();
                } else {
                    $('#did_add_automation_issue_wrapper').hide();
                }
            }

            function toggleFollowSOPYesQuestion() {
                let sopVal = $('input[name="follow_sop"]:checked').val();
                console.log('sopVal' , sopVal);
                if (sopVal == '1') {
                    $('#follow_sop_yes_question_wrapper').show();
                    $("#follow_sop_no_question_wrapper").hide();
                } else if (sopVal == '0'){
                    $('#follow_sop_yes_question_wrapper').hide();
                    $("#follow_sop_no_question_wrapper").show();
                }else{
                    $('#follow_sop_yes_question_wrapper').hide();
                    $("#follow_sop_no_question_wrapper").hide();
                }
            ...
            function toggleFollow_automation_issue_YesQuestion() {
                let did_add_automation_issue = $('input[name="did_add_automation_issue"]:checked').val();
                if (did_add_automation_issue == '1') {
                    $('#automation_issue_yes_question_wrapper').show();
                    $("#automation_issue_no_question_wrapper").hide();
                } else if (did_add_automation_issue == '0'){
                    $('#automation_issue_yes_question_wrapper').hide();
                    $("#automation_issue_no_question_wrapper").show();
                }else{
                    $('#automation_issue_yes_question_wrapper').hide();
                    $("#automation_issue_no_question_wrapper").hide();
                }
            ...
            $(document).on('change', 'input[name="did_add_automation_issue"]', function () {
                toggleFollow_automation_issue_YesQuestion();
            });
            ...
            $(document).on('shown.bs.modal', '#_task_modal', function () {
                ...
                // Check if modal has "edit" class
                if (!$modal.hasClass('edit')) {

                    // remove checked state from radio buttons
                    $modal.find('input[name="follow_sop"]').prop('checked', false);
                    $('#follow_sop_yes_question_wrapper').hide();
                    $('#follow_sop_no_question_wrapper').hide();

                    $modal.find('input[name="did_add_automation_issue"]').prop('checked', false);
                    $('#automation_issue_yes_question_wrapper').hide();
                    $('#automation_issue_no_question_wrapper').hide();
                }else{
                    let val10 = $('[name="custom_fields[tasks][10]"]').val();
