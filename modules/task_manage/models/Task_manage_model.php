...
class Task_manage_model extends CI_Model
{
    ...
    public function __construct()
    {
        ...
        $this->load->helper(TASK_MANAGE_MODULE_NAME . '/' . TASK_MANAGE_MODULE_NAME);
    }
    ...
    public function get_groups($return_array = false)
    {
        ...
        if ($return_array)
            return $this->db->select('*')->from($this->table_group)->where('status', 1)->order_by('group_name', 'asc')->get()->result_array();

        return $this->db->select('*')->from($this->table_group)->where('status', 1)->order_by('group_name', 'asc')->get()->result();
    }
    ...
    private function create_project_task($project_id, $product_task, $milestone_converts_id)
    {
        ...
        if ($project_task_id) {
            ...
            // jay code
            // okay so what he want is 
            // first get what he need like cam or any other 
            // then get project related client and then that client related cam id which is basicaly a staff id and then that will be assign to task 

            $get_what_related = '';
            if ($product_task->assign_project_owner == 1) {
                $get_what_related = 'cam_id';
            } elseif ($product_task->assign_project_owner == 2) {
                $get_what_related = 'optimizer_id';
            } elseif ($product_task->assign_project_owner == 3) {
                $get_what_related = 'organic_social_id';
            } elseif ($product_task->assign_project_owner == 4) {
                $get_what_related = 'seo_lead_id';
            } elseif ($product_task->assign_project_owner == 5) {
                $get_what_related = 'sale_rep_id';
            } elseif ($product_task->assign_project_owner == 6) {
                $get_what_related = 'content_id';
            } elseif ($product_task->assign_project_owner == 7) {
                $get_what_related = 'web_lead_id';
            } else {
                $get_what_related = which_type_assign_to_task($product_task->assign_project_owner);
            }
            if ($get_what_related != '') {
                if ($product_task->assign_project_owner < 7 && $product_task->assign_project_owner > 0) {
                    $project_client_id = $this->db->select('clientid')->from(db_prefix() . 'projects')->where('id', $project_id)->get()->row();
                    $project_client = $this->db->select($get_what_related)->from(db_prefix() . 'clients')->where('userid', $project_client_id->clientid)->get()->row();
                    if ($project_client->$get_what_related != '') {
                        if (!empty($project_client->$get_what_related) && $product_task->assign_project_owner != 0) {
                            $this->db->insert(db_prefix() . 'task_assigned', ['staffid' => $project_client->$get_what_related, 'taskid' => $project_task_id]);
                        }
                    } else {
                        // if there are no related company management then we will follow fallback rule which is get that staff which has related roles 
                        $get_what_related_staff_roles = '';
                        if ($product_task->assign_project_owner == 1) {
                            $get_what_related_staff_roles = 'CAM Director';
                        } elseif ($product_task->assign_project_owner == 2) {
                            $get_what_related_staff_roles = 'Paid Ads Director';
                        } elseif ($product_task->assign_project_owner == 3) {
                            $get_what_related_staff_roles = 'Social Media Manager';
                        } elseif ($product_task->assign_project_owner == 4) {
                            $get_what_related_staff_roles = 'SEO Director';
                        } elseif ($product_task->assign_project_owner == 5) {
                            $get_what_related_staff_roles = 'Sales Director';
                        } elseif ($product_task->assign_project_owner == 6) {
                            $get_what_related_staff_roles = 'Content Director';
                        } elseif ($product_task->assign_project_owner == 7) {
                            $get_what_related_staff_roles = 'Web Director';
                        }

                        if ($get_what_related_staff_roles != '') {
                            //    $get_all_staff_related_roles = $this->db->select($get_what_related)->from(db_prefix() . 'customfieldsvalues')->where('userid', $project_client_id->clientid)->get()->row();
                            $this->db->select('relid');
                            $this->db->where('fieldid', STAFF_ROLES);
                            $this->db->where('fieldto', 'staff');
                            $this->db->where('value', $get_what_related_staff_roles);
                            $get_all_staff_related_roles = $this->db->get(db_prefix() . 'customfieldsvalues')->result_array();

                            foreach ($get_all_staff_related_roles as $key => $value) {
                                if ($value['relid'] != '') {
                                    $this->db->insert(db_prefix() . 'task_assigned', ['staffid' => $value['relid'], 'taskid' => $project_task_id]);
                                }
                            }
                        }
                    }
                } else {
                    if ($product_task->assign_project_owner > 7) {
                        $this->db->select('relid');
                        $this->db->where('fieldid', STAFF_ROLES);
                        $this->db->where('fieldto', 'staff');
                        $this->db->like('value', $get_what_related);
                        $get_all_staff_related_roles = $this->db->get(db_prefix() . 'customfieldsvalues')->result_array();
                        foreach ($get_all_staff_related_roles as $key => $value) {
                            if ($value['relid'] != '') {
                                $this->db->insert(db_prefix() . 'task_assigned', ['staffid' => $value['relid'], 'taskid' => $project_task_id]);
