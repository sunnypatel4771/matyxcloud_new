        ...
        <select name="assignees[]" id="assignees" class="selectpicker" data-width="100%"
            ...
            <?php foreach ($members as $member) {
                ...
                if (!empty($data->assignees)) {
                    $assignees_data = json_decode($data->assignees, true);
                    if (is_array($assignees_data) && in_array($member["staffid"], $assignees_data))
                        $selected = "selected";
                }
...
<div class="col-md-6 mtop10">
    ...
    <?php
    echo render_select('assign_project_owner', which_type_assign_to_task(), ['id', 'name'], 'Assign to Role', $data->assign_project_owner);
    ?>
        ...
        <select id="checklist_items" name="checklist_items[]"
            ...
            <?php foreach ($checklistTemplates as $chkTemplate) {
                ...
                if (!empty($data->checklist_items)) {
                    ...
                    //     $selected = "selected";
                    $checklist_items_data = json_decode($data->checklist_items, true);
                    if (is_array($checklist_items_data) && in_array($chkTemplate["id"], $checklist_items_data)) {
                        $selected = "selected";
                    }
                }
            ?>
