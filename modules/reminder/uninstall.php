...
if ($CI->db->field_exists('user_type', db_prefix() . 'reminders')) {
  $CI->db->query("ALTER TABLE `" . db_prefix() . "reminders` DROP `user_type`");
}
if (total_rows(db_prefix() . 'emailtemplates', ['slug' => 'reminder-send-to-lead'])) {
  $CI->db->where('slug', 'reminder-send-to-lead');
  $CI->db->delete(db_prefix() . 'emailtemplates');
}
if (total_rows(db_prefix() . 'emailtemplates', ['slug' => 'reminder-service-send-to-lead'])) {
  $CI->db->where('slug', 'reminder-service-send-to-lead');
  $CI->db->delete(db_prefix() . 'emailtemplates');
}
