...
if(!is_admin()){
    if (staff_can('view_own', 'reminder')) {
        array_push($where, ' AND ' . db_prefix() . 'reminders.created_by_staff=' . $staff_id);
    }
}
...
$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [db_prefix() . 'reminders.id as id', db_prefix() . 'reminders.user_type']);
// print_r($result); die;
...
foreach ($rResult as $aRow) {
    ...
    if($aRow['user_type'] == 'lead'){
        $row[] = rm_get_lead($aRow['customer'])->name;
    }else{
