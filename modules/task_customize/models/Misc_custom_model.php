...
class Misc_custom_model extends App_Model
{
    ...
    public function _search_proposals($q, $customer_id, $limit = 0)
    {
        $result = [
            'result'         => [],
            'type'           => 'proposals',
    ...
    public function _search_leads($q, $customer_id, $limit = 0, $where = [])
    {
        $result = [
            'result'         => [],
            'type'           => 'leads',
    ...
    public function _search_tickets($q, $customer_id, $limit = 0)
    {
        $result = [
            'result'         => [],
            'type'           => 'tickets',
        ...
        if (is_staff_member() || (!is_staff_member() && get_option('access_tickets_to_none_staff_members') == 1)) {
            ...
            if ($customer_id != '') {
                $this->db->where(db_prefix() . 'tickets.client_id', $customer_id);
            }
    ...
    public function _search_contracts($q, $customer_id, $limit = 0)
    {
        $result = [
            'result'         => [],
            'type'           => 'contracts',
        ...
        if ($has_permission_view_contracts || staff_can('view_own',  'contracts')) {
            ...
            if ($customer_id != '') {
                $this->db->where(db_prefix() . 'contracts.client', $customer_id);
            }
    ...
    public function _search_invoices($q, $customer_id, $limit = 0)
    {
        $result = [
            'result'         => [],
            'type'           => 'invoices',
    ...
    public function _search_credit_notes($q, $customer_id, $limit = 0)
    {
        $result = [
            'result'         => [],
            'type'           => 'credit_note',
        ...
        if ($has_permission_view_credit_notes || $has_permission_view_credit_notes_own) {
            ...
            if ($customer_id != '') {
                $this->db->where(db_prefix() . 'creditnotes.clientid', $customer_id);
            }
    ...
    public function _search_estimates($q, $customer_id, $limit = 0)
    {
        $result = [
            'result'         => [],
            'type'           => 'estimates',
        ...
        if ($has_permission_view_estimates || $has_permission_view_estimates_own || get_option('allow_staff_view_estimates_assigned') == '1') {
            ...
            if ($customer_id != '') {
                $this->db->where(db_prefix() . 'estimates.clientid', $customer_id);
            }
    ...
    public function _search_expenses($q, $customer_id, $limit = 0)
    {
        $result = [
            'result'         => [],
            'type'           => 'expenses',
        ...
        if ($has_permission_expenses_view || $has_permission_expenses_view_own) {
            ...
            if ($customer_id != '') {
                $this->db->where(db_prefix() . 'expenses.clientid', $customer_id);
            }
