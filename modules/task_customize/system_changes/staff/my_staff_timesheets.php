...
if ($v && strpos($v->version, '5.7') !== false) {
    ...
    $aColumns = array_values(array_filter([
        ...
        'ANY_VALUE(' . db_prefix() . 'tasks.billable) as billable',
'ANY_VALUE((SELECT value FROM ' . db_prefix() . 'customfieldsvalues 
   WHERE relid = ' . db_prefix() . 'tasks.id 
   AND fieldid = '. TASK_TYPE .' 
   AND fieldto = "tasks")) as custom_field_10',
    ...
    $aColumns = array_values(array_filter([
        ...
        db_prefix() . 'tasks.billable as billable',
'(SELECT value FROM ' . db_prefix() . 'customfieldsvalues 
   WHERE relid = ' . db_prefix() . 'tasks.id 
   AND fieldid = '. TASK_TYPE .' 
   AND fieldto = "tasks") as custom_field_10',
...
$billable_filter = $this->ci->input->post('billable_filter', true);

if ($billable_filter === 'yes') {
    $where[] = 'AND ' . db_prefix() . 'tasks.billable = 1';
} elseif ($billable_filter === 'no') {
    $where[] = 'AND ' . db_prefix() . 'tasks.billable = 0';
}

$cf_filter = $this->ci->input->post('custom_field_10_filter', true);

if (!empty($cf_filter)) {

    $where[] = 'AND EXISTS (
        SELECT 1 FROM ' . db_prefix() . 'customfieldsvalues
        WHERE relid = ' . db_prefix() . 'tasks.id
        AND fieldid = '. TASK_TYPE .' 
        AND fieldto = "tasks"
        AND value = "' . $this->ci->db->escape_str($cf_filter) . '"
    )';
}
...
foreach ($rResult as $aRow) {
    ...
    $row[] = $aRow['billable'] == 1 ? 'Yes' : 'No';
    $row[] = e($aRow['custom_field_10']);
