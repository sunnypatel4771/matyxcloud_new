...
hooks()->add_action('app_admin_footer', 'task_customize_footer_components');
...
function task_customize_module_activation_hook()
{
    ...
    $module_my_filters_path = module_dir_path(TASK_CUSTOMIZE_MODULE_NAME) . 'system_changes/task/my_filters.php';
...
function task_customize_hook_app_admin_footer()
{
    ...
    echo '<script src="' . module_dir_url(TASK_CUSTOMIZE_MODULE_NAME, 'assets/js/mentions-filter.js') . '?v=' . VERSION_TASK_CUSTOMIZE . '"></script>';
}
...
function task_customize_after_add_task($data)
{
    ...
    $taskdata = $CI->db->get_where(db_prefix() . 'tasks', ['id' => $data])->row();
    if (isset($taskdata->priority) && $taskdata->priority == 4) {
        //update tbltask is_poked=1
        $CI->db->where('id', $taskdata->id);
        $CI->db->update(db_prefix() . 'tasks', [
            'is_poked' => 1
        ]);
    }
...
function task_customize_footer_components()
{
    echo "<script>
    $(document).on('click', '.task-comment .comment-content a', function(e) {
        e.preventDefault();
...
// task_priority_changed_controller
hooks()->add_action('after_update_task', 'task_customize_task_priority_changed', 10, 2);
function task_customize_task_priority_changed($id)
{
    $CI = &get_instance();
    $taskdata = $CI->db->get_where(db_prefix() . 'tasks', ['id' => $id])->row();
    if (isset($taskdata->priority) && $taskdata->priority == 4) {
        //update tbltask is_poked=1
        $CI->db->where('id', $taskdata->id);
        $CI->db->update(db_prefix() . 'tasks', [
